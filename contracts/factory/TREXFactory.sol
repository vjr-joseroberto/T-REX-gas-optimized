...
contract TREXFactory is ITREXFactory, Ownable {
    ...
    external override onlyOwner {
        ...
        // ORIG: for (uint256 i = 0; i < (_claimDetails.claimTopics).length; i++)
        // OPT: Use unchecked increments. Length caching avoided here to prevent stack too deep in this massive function.
        for (uint256 i = 0; i < _claimDetails.claimTopics.length; ) {
            ctr.addClaimTopic(_claimDetails.claimTopics[i]);
            unchecked { ++i; }
        }
        
        for (uint256 i = 0; i < _claimDetails.issuers.length; ) {
        ...
        for (uint256 i = 0; i < _tokenDetails.irAgents.length; ) {
            AgentRole(address(ir)).addAgent(_tokenDetails.irAgents[i]);
            unchecked { ++i; }
        }
        
        for (uint256 i = 0; i < _tokenDetails.tokenAgents.length; ) {
            AgentRole(address(token)).addAgent(_tokenDetails.tokenAgents[i]);
            unchecked { ++i; }
        }
        
        for (uint256 i = 0; i < _tokenDetails.complianceModules.length; ) {
